...
canvas.addEventListener("click", function () {
  ...
  const sqrY = pointer.y - (pointer.y % square_size); // resolve the collision overlapping
  if (sqrY < square_size) return;
  for (let i = 0; i < plants.length; i++) {
    if (plants[i].x == sqrX && plants[i].y == sqrY) return;
  }
...
function managePlants() {
  for (let i = 0; i < plants.length; i++) {
    plants[i].draw();
    if (plants[i].type != "wallnut") plants[i].bringPease();
    if (zombiePos.indexOf(plants[i].y) !== -1) plants[i].zombiedetect = true;
    else plants[i].zombiedetect = false;
...
function sunese() {
  for (let i = 0; i < rsuns.length; i++) {
    ...
    if (
      !rsuns[i].picked &&
      mouseinsqr(pointer.x, pointer.y, rsuns[i].rx, rsuns[i].ry, 60)
    ) {
      rsuns[i].pick();
      tsun += rsuns[i].power;
      rsuns.splice(i, 1);
...
function collision(a, b) {
  return !(
    a.x > b.x + b.side ||
    a.x + a.side < b.x ||
    a.y > b.y + b.side-30 ||    //leg and plant collision
    a.y + a.side-30 < b.y      //  head and plant collision bug resolved
  );
}
function updateGame() {
